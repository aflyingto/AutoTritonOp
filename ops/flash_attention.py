...
This is a Triton-Ascend implementation of the Flash Attention v2 algorithm.
"""
...
def _attn_fwd_inner(acc_ptr, l_i, m_i, q,  # Accumulator, local l, local m, query vector
    ...
    # and cannot access access information from future positions.
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype, BM, BN):
    # Filter non-divisible cases, N_CTX must be divisible by BM and BN, and HEAD_DIM must be divisible by 16
    if N_CTX % BM != 0 or N_CTX % BN != 0 or HEAD_DIM % 16 != 0:
        print(f"Skipping non-divisible case: N_CTX={N_CTX}, BM={BM}, BN={BN}, HEAD_DIM={HEAD_DIM}")
        return
    ...
    try:
        ...
        print(f"Test Fused-Attention PASS! ({Z},{H},{N_CTX},{HEAD_DIM}), causal={causal}, dtype={dtype}, BM={BM}, BN={BN}")
